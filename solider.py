...
def getWeather():
    ...
    if a[3][-3:-1][0] == '"':
        ...
        weather = a[3][-3:-1]
        # print(a[3][-3:-1])
    conn = pymysql.connect(host='localhost', user='user',passwd='passwd', db='tableName')
    c = conn.cursor()
    ...
    for i in tmp:
        ...
        # bot.sendMessage(int(list(i)[0]), cmd1[nowCmd] + cmd2[nowCmd] + cmd3[nowCmd])
    boardcast()
...
def handle(msg):
    global cmd, nowCmd, conn, nowNew, newS, newD, newI ,weather
...
global cmd1 , cmd2 , cmd3 ,todayCMD
...
schedule.every(15).minutes.do(decreaseWaterPeriod)
print ('Listening ...')
